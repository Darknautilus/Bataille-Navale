 ...
 *
 * DÈfinit le type du bateau et le nom du type (vise ‡ remplacer ETypeBat)
*/
typedef struct
{
...
typedef struct
{
    ...
    int estPlace; /**< Vaut 1 si le bateau est placé sur sa grille et 0 sinon */
}
TBateau;
...
/**
 * \brief RÈcupère un pointeur sur le bateau dÈsignÈ par l'id
 *
 * \param[in] idBateau L'id du bateau dÈsirÈ
 * \return Un pointeur sur le bateau dÈsirÈ
*/
...
/**
 * Determine si un bateau est plaÁable ou non.
 * \param[in] bat Le bateau en question.
 * \param[in] grille La grille qui dÈsire contenir le bateau.
 * \return 1 si le bateau est plaÁable, 0 sinon.
*/
